...
public:
    ...
    uint8_t* transaction_id_ = nullptr;//12 bytes
...
public:
	...
	bool xor_address_allocated_ = false;
...
public:
    StunPacket();
    StunPacket(uint8_t* data, size_t len);
    ~StunPacket();
...
public:
    ...
    static StunPacket* Parse(uint8_t* data, size_t len);
...
*/
public:
    STUN_AUTHENTICATION CheckAuthentication(const std::string& ufrag, const std::string& pwd);
    int Serialize();
    std::string Dump();
